...
// Landing page propia de una toximoneda válida (destino final tras el 302).
export function renderProfilePage(id, moneda) {
  const lote = moneda.lote_emision ? `Lote ${escapeHtml(moneda.lote_emision)}` : '';
  return `<!DOCTYPE html>
<html lang="es">
<head>
<meta charset="UTF-8" />
<meta name="viewport" content="width=device-width, initial-scale=1.0" />
<title>Moneda TOXI #${escapeHtml(id)} · TOXI MEDIA</title>
<style>${BASE_STYLE}
  .badge{margin-top:24px;font-size:0.8rem;letter-spacing:0.1em;text-transform:uppercase;opacity:0.5}
</style>
</head>
<body>
<main>
  <div class="brand">TOXI MEDIA</div>
  <h1>Moneda TOXI #${escapeHtml(id)} — Verificada</h1>
  <p>Esta toximoneda física es auténtica. Muy pronto esta página va a mostrar contenido exclusivo.</p>
  ${lote ? `<div class="badge">${lote}</div>` : ''}
</main>
</body>
</html>`;
}
